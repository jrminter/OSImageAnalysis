...
# @Integer(label = "Image Width", value=1024) imgwidth
# @String(label = "kV", value="5.0") kv
# @Integer(label = "spot", value=3) spot
...
# @String(label = "mag (X)", value="100.0") mag
...
fScale = estimateAztecScaleFactorX(fMag, imgwidth, slope=289251.80, slopeSE=16.54, rDigits=7)
fScaleX = fScale[0]
...
fScaleY = computeTiltScaleFactorY(fScaleX, fTilt)
strLine = "ScaleY=%.6f" %  fScaleY + "\n"
f.write(strLine)
strLine = "Units=µm\n"
f.write(strLine)
strLine = "Comment=%g kV, S%d, %.1f mm, %s, %.1f deg tilt, %s, %s\n\n" % (fkV, spot, fFwd, det, fTilt, scan, comment )
f.write(strLine)

f.close()

strMsg = "Processed image %s at magnification %.1fX" % (basename, fMag)
print(strMsg)
