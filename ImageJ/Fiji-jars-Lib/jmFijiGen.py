...
def reduceMapNoise(imp):
	"""reduceMapNoise(imp)
	Reduce the nois in an X-Ray EDS map using an ROF denoise.
	Input:
	imp - an Image plus"""
	IJ.run(imp, "32-bit", "")
	IJ.run(imp, "ROF Denoise", "theta=25")
	IJ.run(imp, "16-bit", "")
...
def detectEdges(imp):
	...
	IJ.run(imp, "Square", "")	# turns the gradient into gradient squared
	IJ.run(imp, "Square", "")	# further enhances the good edges
	IJ.run(imp, "Enhance Contrast", "saturated=0")
	IJ.run(imp, "8-bit", "")
...
def detectMapLine(imp, thrMethod="Default dark"):
	...
	This detects a line in an X-ray EDS map and converts the
	input ImagePlus to a Mask."""
	IJ.setAutoThreshold(imp, thrMethod)
	IJ.run(imp, "Convert to Mask", "")

def analyzeRois(imp, vAvg, outPath):
	"""analyzeRois(imp, vAvg, outPath)
	This analyzes the calibrated input ImagePlus (imp) containing two
