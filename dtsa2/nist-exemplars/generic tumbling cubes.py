...
basePath = "%s/%s" % (DefaultOutput, mat)
jio.File("%s" % basePath).mkdirs()
...
for e0 in [10,15,20,25,30]:
   if terminated:
      break
   out = textFile("%s/output[E=%g keV].csv" % (basePath, e0))
   ...
   for i in range(0,8):
      header = "%s\t" % header
      for elm in mat.getElementSet():
          header = "%s\t%2.5g" % (header, mat.weightFraction(elm,true)*100.0)
   out.println(header)
     
   quant = epq.QuantifySpectrumUsingStandards(det, epq.ToSI.keV(e0))
   for elm in mat.getElementSet():
       quant.addStandard(elm,mat,computeBulk(mat, e0))
   ...
   for size in [0.1e-6, 0.2e-6, 0.4e-6, 0.8e-6, 1.6e-6, 3.2e-6, 6.4e-6]:
        ...
        for i in range(0,37):
            ...
            path = "%s/E=%g keV/size=%g um" % (basePath, e0, size*1.0e6)
            jio.File(path).mkdirs()
            ...
            monte.addSubRegion(monte.getChamber(),mat,nm.MultiPlaneShape.createBlock([size, size, size],center,phi,th,psi))
            ...
            try:
                for sh in shapes:
                   ...
                   str = "%s\t%s" % (str, sh)
                   for elm in mat.getElementSet():
                      str = "%s\t%g" % (str,100.0*(q.weightFraction(elm,true)-mat.weightFraction(elm,true)))
                out.println(str)
            except:
