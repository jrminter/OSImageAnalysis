...
dumpComp(comp):  Write the composition to human readable format.'''
...
if populate:   
   def addToDB(list):
      for comp in list:
         if not dtsa2.DTSA2.getSession().findStandard(comp.getName()):
            dtsa2.DTSA2.getSession().addStandard(comp)
   ...
   Fluorapatite = safematerial("Ca5(PO3)4F")
   Fluorapatite.setName('Fluorapatite')

   addToDB([Anorthite,Albite,Diopside,Apatite,Fluorapatite])
   ...
   K1228=epq.Composition([epq.Element.Li, epq.Element.Na, epq.Element.Al, epq.Element.Si, epq.Element.K, epq.Element.Ca, epq.Element.Rb, epq.Element.Cs, epq.Element.Ba, epq.Element.O],[0.0139, 0.0223, 0.0794, 0.1870, 0.0249, 0.1072, 0.0274, 0.0283, 0.1343, 0.3753],'K1228')
...
def dumpComp(comp):
   ...
   if isinstance(comp,list) or isinstance(comp,tuple):
       comp=map(material,comp)
       elms=[]
       for item in comp:
           for elm in item.getElementSet():
               if not (elm in elms):
                   elms.append(elm)
       str = "Z\tElm" 
       end = "---\t---"
       ...
       print str
       for elm in elms:
           str="%d\t%s" % (elm.getAtomicNumber(),elm.toAbbrev())
           for item in comp:
               str="%s\t%0.2f" % (str, 100.0*item.weightFraction(elm,0))
           print str
   else:
        comp=material(comp)
...
def stdToPython(comp):
    comp=material(comp)
    elms = ",".join("epq.Element.%s" % elm.toAbbrev() for elm in comp.getElementSet())
    qty = ",".join("%f" % comp.weightFraction(elm,0) for elm in comp.getElementSet())
    return "%s=epq.Composition([%s],[%s],\'%s\')" % (comp.getName().replace(' ','_'),elms,qty,comp.getName())

def missingElements(unk,std,min=1.0):
    ...
    missing=[]
    for elm in unk:
        if 100.0*std.weightFraction(elm)<min:
            missing.append(elm)
    return missing
