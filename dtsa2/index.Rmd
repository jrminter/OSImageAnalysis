...
2. Go into Preferences and select the ``Instruments and Detector`` folder icon and the ``Add`` button. Click the dialog box and paste in ``FEI FIB 620`` in the name with a kV range from 5 to 30 kV. Import the ``2-FEI FIB 620 EDAX-RTEM.xdet`` detector. ***Restart DTSA*** to write the database.
3. Go into Preferences and select the ``Instruments and Detector`` folder icon and the ``Add`` button. Click the dialog box and paste in ``FEI CM20UT`` in the name with a kV range from 40 to 200 kV. Import the ``2-FEI CM20UT EDAX-RTEM.xdet`` detector. ***Restart DTSA*** to write the database.
...
5. Start DTSA and select the ``Command`` pane. Open the Jython script ``stds.py`` from ``$GIT_HOME/OSImageAnalysis/dtsa2/productionScripts`` directory to load in the standards.
...
[FEI FIB620 EDAX-RTEM, NIST JXA8500F Brucker-SDD, Si(Li), FEI CM20UT EDAX-RTEM]
Elapse: 0:00:00.0
2> listDetectors()
Name  Detector
...
{FEI FIB 620=2, Probe=1, NIST JXA8500F=4, FEI CM20UT=3}
4> listMaterials()
[Al2O3, Cu standard, Fe standard, FeNi, K411, K412, K496, Mn standard, NaCl, Ni, Pd, SRM-484, SS-316, Zn standard]
5> 
