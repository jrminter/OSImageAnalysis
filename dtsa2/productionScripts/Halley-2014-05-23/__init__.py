   ...
   try:
      for xrt in range(epq.XRayTransition.KA1, epq.XRayTransition.N5N6 + 1):
         if epq.XRayTransition.exists(elm, xrt):
            ...
            print "%s\t%s\t%0.4f\t%g\t%g" % (tr.toString(), tr.getSiegbahnName(), tr.getWeight(epq.XRayTransition.NormalizeKLM), epq.FromSI.keV(tr.getEnergy()), 1.0e10 * epq.SpectrumUtils.energyToWavelength(tr.getEnergy()))
...
def tabulate(specs, withErrs=False, normalize=False, prop=epq.SpectrumProperties.MicroanalyticalComposition, precision=4, massFrac=True):
